...
class SearchRequest(BaseModel):
    ...
    limit: int | None = Field(
        default=None, description="Optional max number of results."
    )
...
class SearchResult(BaseModel):
    ...
    tags: list[str]
    score: float = Field(
        description="Hybrid relevance score after reciprocal rank fusion."
    )
    ...
    dense_score: float | None = Field(
        default=None, description="Raw dense retrieval score, when available."
    )
    ...
    excerpt: str = Field(
        description="Short preview of the matching chunk for quick inspection."
    )
    content: str
